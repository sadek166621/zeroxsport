                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <td>
                                    <div class="order-date">{{ \Carbon\Carbon::parse($order->created_at)->format('d M Y') }}
                                    </div>
                                    <div class="order-time">{{ \Carbon\Carbon::parse($order->created_at)->format('h:i A') }}
                                    </div>
                                </td>
                                <td>
                                    @php
                                        ...
                                        $status = $statusMap[$order->delivery_status] ?? [
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        @if ($order->delivery_status == 4 && $order->hasReplaceableProduct())
                                            <button type="button" class="btn-action btn-return"
                                                data-check-url="{{ route('order.return.check', $order->id) }}">
                ...
                success: function(data) {
                    if (data.result) {
                        window.location.href =
                            "{{ url('order/return') }}/" + data.order.invoice_no;
                    } else {
                        Swal.fire({
                            toast: true,
                            position: 'top-end',
